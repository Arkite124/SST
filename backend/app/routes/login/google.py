...
async def google_callback(request: Request, db: Session = Depends(get_db)):
    ...
    if not email:
        raise HTTPException(status_code=400, detail="구글 이메일 등록여부를 확인해주세요.")

    # ✅ DB 조회
    user = db.query(User).filter(User.email == user_info["email"]).first()

    if not user:
        user = User(email=email, name=user_info.get("name"), oauth="google")
        db.add(user)
        db.commit()
        db.refresh(user)
    ...
    access_token = create_access_token(user.id, expires_minutes=60)   # 60분짜리
