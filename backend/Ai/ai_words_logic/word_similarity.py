...
from backend.Ai.db.pg_connect import extract_similar_words
    ...
    if model is None:
        if not os.path.exists(model_path):
            raise FileNotFoundError(
                f"모델 디렉토리 '{model_path}'가 존재하지 않습니다. "
                f"먼저 run_training()을 실행하여 모델을 학습시켜주세요."
            )
        device = get_device_cuda()
        model = SentenceTransformer(model_path, device=device)
        print(f"[Info] 모델 로드 완료 ({device})")
    return model, device
...
# 저장된 모델 불러오기 + 검색 전용
# -----------------------------
def load_model_and_corpus(emb_path=EMB_PATH):
...
if __name__ == "__main__":
    ...
    candidates = similar_fn(word_base1, topk=2)
    candidates2 = similar_fn(word_base2, topk=2)
    candidates3 = similar_fn(word_base3, topk=2)
    end_time = time.time()
    print(candidates)
