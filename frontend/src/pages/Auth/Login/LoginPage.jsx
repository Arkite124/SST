...
export default function LoginPage() {
    ...
    return (
        <div
            className="font-juache relative w-full py-10 px-4 text-center bg-[#F8FAF4] backdrop-blur-md shadow-inner snap-start flex flex-col items-center justify-center"
            style={{
                height: 'calc(100vh)',
            }}
        >
                    ...
                    <form onSubmit={onSubmit} className="space-y-2 text-left">
                        <div>
                            ...
                            <input name="email" type="email" value={form.email} onChange={onChange}
                                   className="w-full border-2 border-[#B4E197] rounded-xl p-1.5 px-3 focus:ring-2 focus:ring-[#83BD75]" />
                        </div>
                        <div>
                            <label className="block text-sm text-[#4E944F] mb-1">비밀번호</label>
                            <input name="password" type="password" value={form.password} onChange={onChange}
                                   className="w-full border-2 border-[#B4E197] rounded-xl p-1.5 px-3 mb-3 focus:ring-2 focus:ring-[#83BD75]" />
                        </div>
                        <button type="submit" disabled={loading}
                                className={`w-full rounded-xl text-xl text-white py-1.5 ${loading ? "bg-gray-400" : "bg-[#4E944F] hover:bg-[#3a7a3d]"}`}>
                            {loading ? "로그인 중..." : "로그인"}
                        </button>
                    </form>
                    ...
                    <div className="flex flex-col items-center space-y-3 my-3">
                        ...
                        >
                            <img
                                ...
                                alt="구글 로그인"
                                className="w-[14rem] h-10 object-cover rounded"
                            />
                        </a>
